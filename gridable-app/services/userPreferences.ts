import {
  getKeyValueDatabase,
  // getDocumentStoreDatabase, // Not directly used here, but view defs are doc stores
} from './orbitdb';
import { useAuthStore } from '~/store/auth';
import { CustomDIDAccessController } from './orbitdb-did-access-controller';
import { mockDidVerificationFunction } from './orbitdb'; // Import mock for AC options

// --- Data Structures (existing) ---
export interface GlobalAppSettings { defaultThemeId?: string; defaultItemsPerPage?: number; }
...
function getUserPrivateAccessControllerOptions(): any | null {
  ...
  return {
    ...
    // verificationFunction is supplied by services/orbitdb.ts when it instantiates the AC
  };
}
...
export async function getGlobalAppSettings(): Promise<GlobalAppSettings | null> {
  ...
  try { const db = await getKeyValueDatabase(userPrefsDbName, acOptions); const settings = await db.get("globalAppSettings"); return settings || {} as GlobalAppSettings; }
...
export async function saveGlobalAppSettings(settings: GlobalAppSettings): Promise<void> {
  ...
  try { const db = await getKeyValueDatabase(userPrefsDbName, acOptions); await db.put("globalAppSettings", settings); }
...
export async function getViewGridState(viewOrbitDbAddress: string): Promise<GridState | null> {
  ...
  const acOptions = getUserPrivateAccessControllerOptions(); if (!acOptions) return null;
  try { const db = await getKeyValueDatabase(viewSettingsDbName, acOptions); const state = await db.get("gridState"); return state || {} as GridState; }
...
export async function saveViewGridState(viewOrbitDbAddress: string, gridState: GridState): Promise<void> {
  ...
  const acOptions = getUserPrivateAccessControllerOptions(); if (!acOptions) return;
  try { const db = await getKeyValueDatabase(viewSettingsDbName, acOptions); await db.put("gridState", gridState); }
...
async function getTablesManifest(): Promise<Record<string, TableMetadata>> {
  const userPrefsDbName = getUserPrefsDbName(); if (!userPrefsDbName) return {};
  const acOptions = getUserPrivateAccessControllerOptions(); if (!acOptions) return {};
  try { const db = await getKeyValueDatabase(userPrefsDbName, acOptions); const m = await db.get(TABLES_MANIFEST_KEY); return m || {}; }
...
async function saveTablesManifest(manifest: Record<string, TableMetadata>): Promise<void> {
  const userPrefsDbName = getUserPrefsDbName(); if (!userPrefsDbName) return;
  const acOptions = getUserPrivateAccessControllerOptions(); if (!acOptions) return;
  try { const db = await getKeyValueDatabase(userPrefsDbName, acOptions); await db.put(TABLES_MANIFEST_KEY, manifest); }
...
const SAVED_VIEWS_MANIFEST_KEY = 'savedViewsManifest';
